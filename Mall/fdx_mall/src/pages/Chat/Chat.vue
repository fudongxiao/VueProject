    ...
    <div v-if="this.userInfo.id">
        ...
        <section>
          <div class="shopCart_list_con" v-for="(goods, index) in cartGoods" :key="index">
            <div class="list_con_left">
              <a href="javascript:;" class="cart_check_box" :checked="goods.checked" @click.stop="selectedGoods(goods)"></a>
            </div>
              ...
              <div class="shop_con">
                ...
                <div class="shop_deal">
                  <div class="shop_deal_left">
                    <span @click.stop="updataGoodsCount(goods,false)">-</span>
                    <input type="tel" value="1" v-model="goods.buy_count" disabled>
                    <span @click.stop="updataGoodsCount(goods,true)">+</span>
                  </div>
      ...
      <div id="tab_bar">
        <div class="tab-bar-left">
          <a href="javascript:;" class="cart_check_box" :checked="isSelectAll" @click.stop="selectedAll(isSelectAll)"></a>
...
export default {
  ...
  mounted() {
      //1. 请求商品数据
      this.$store.dispatch('reqCartGoodsList');
    },
    methods:{
      updataGoodsCount(goods,isAdd){
        console.log(goods,isAdd);
        this.$store.dispatch("updataGoodsCount",{goods,isAdd});
        //计算商品总价格
      ...
      //4.单个商品的是否选中
      selectedGoods(goods){
        this.$store.dispatch('selectedGoods',{goods});
         //计算商品总价格
        this.getAllGoodsPrice();
        this.hasSelectAll();
      },
      ...
      //6.点击删除
      clickTrash(goods){
        MessageBox.confirm('主人，您真的不要小的了吗?').then(action => {
            // console.log(action);
            if('confirm' === action){
                 this.deleteGoods = goods;
                 this.$store.dispatch('deleteGoods',{goods});
                  //计算商品总价格
                  this.getAllGoodsPrice();
            }
          });
